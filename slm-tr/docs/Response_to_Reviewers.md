...
* **Author Response:** We have documented the complete training configuration in **Section V-A**: AdamW optimizer ($lr=2	imes 10^{-5}$, weight decay $0.01$), batch size 16, 3 epochs, input truncation $L=128$, sliding window $K=3$, and a composite multi-task objective: $\mathcal{L}_{	ext{total}} = \mathcal{L}_{	ext{CE}} + lpha \mathcal{L}_{	ext{Contrastive}} + eta \mathcal{L}_{	ext{MSE}}$.
