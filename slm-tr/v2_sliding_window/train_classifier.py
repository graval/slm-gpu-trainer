"""
v2 - Sliding Window Sequence Classification Training Pipeline
Fine-tunes encoder SLMs (DeBERTa-v3-small / DistilBERT) on multi-event temporal context windows (K=3..5).
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train v2 (Sliding Window) SLM Classifier")
    ...
    parser.add_argument("--output_dir", type=str, default="models/deberta-lateral-movement-v2_sliding_window", help="Save directory")
    parser.add_argument("--window_size", type=int, default=3, help="Sliding window size (K events)")
...
def main():
    args = parse_args()
    print("=" * 70)
    print(f"   [v2 - SLIDING WINDOW] SLM CLASSIFIER TRAINING PIPELINE (K={args.window_size})   ")
    print("=" * 70)
    
    os.makedirs(args.output_dir, exist_ok=True)
    train_dataset, val_dataset = load_v2_dataset(
        args.csv_path, 
    ...
    print("[*] Starting v2 fine-tuning on temporal window sequences...")
    trainer.train()
    trainer.save_model(args.output_dir)
    tokenizer.save_pretrained(args.output_dir)
    print(f"[+] v2 Model saved successfully to: {args.output_dir}")
